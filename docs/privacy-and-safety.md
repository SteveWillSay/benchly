...
| **Software update check** (Toolbox) | For each of your installed apps that's in Benchly's curated list, an HTTPS request to **that app's own official source** (the vendor's version endpoint, or the project's GitHub Releases API) asking only "what's the latest version?" | No list of your software is sent anywhere, and no third-party aggregator is involved — each request goes straight to the one vendor and carries nothing about you. It reads versions only; it never downloads or installs. Opening a download is a separate click that just opens the vendor's official page in your browser. |
