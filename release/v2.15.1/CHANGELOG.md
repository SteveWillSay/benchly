...
### Added
- **Find Store-app (AppX) remnants.** A new **Cleanup → App remnants** tab surfaces the leftovers
  that uninstalled or half-removed Store / UWP apps leave behind:
  - **Orphaned per-user data folders** — `%LOCALAPPDATA%\Packages\<PackageFamilyName>` directories
    whose package is no longer registered, with the space each is using. Removable to the **Recycle
    Bin** (reversible), and scoped so the removal only ever touches folders under your `Packages`
    directory (a reparse point can't escape it).
  - **Broken package registrations** — packages `Get-AppxPackage` still lists but whose install
    files are gone or whose status isn't "Ok"; clearable with `Remove-AppxPackage` (all-users when
    elevated).
  - Read-first: nothing is removed until you choose. Browser/IE **AppContainer** sandbox folders
    (`cr.sb.*`, `windows_ie_ac_001`, …) that also live under `Packages` are filtered out so they're
    never mistaken for app remnants.

## [2.15.0] — 2026-07-09

### Changed
- **New default appearance: "Precision" (redesign Phase 1).** An editorial / instrument-panel
  design language replaces the old soft-card look as the default — hairline rules and a strict
  grid instead of drop-shadows and bento cards, squared corners, a cool blue-grey neutral ramp,
  and colour reserved for meaning. This is the foundation phase of a larger redesign; a task-first
  **Home** screen, grouped navigation, and progressive-disclosure deep pages follow in v2.15.x.
- **User-selectable accent.** Precision ships with four accents — **Iris** (default), **Coral**,
  **Amber**, **Teal** — chosen from the Appearance menu and remembered per user.
- **Graphite retained as an opt-out.** The original flat dark theme is preserved pixel-for-pixel
  and is one click away in the Appearance menu (`data-theme="graphite"` / `--theme graphite`).
  Frosted Glass and Chevron are unchanged.
- **Zero-flash launch.** The saved theme and accent are now mirrored to `localStorage` and painted
  synchronously before the first frame, so switching the default to Precision never flashes the
  previous theme on relaunch.

### Notes
- Scope of this phase is the design system + theme plumbing only — every page, tool and shortcut is
  unchanged, and the number-key page shortcuts, Ctrl+K palette, tooltips and hide-until-elevated
  hardening are all preserved. The bespoke icon set, Home screen and PowerShell consolidation land
  in the following phases. Light mode is deferred to a later phase (Precision is dark-first for now).

## [2.14.1] — 2026-06-27

### Security
A hardening patch from a NIST SSDF / pentest-style review (no feature changes). None of these were
remotely exploitable — the bridge is local-only and the UI is XSS-clean — but they were real
defects worth removing. Filed and tracked as issues #3–#6.
- **Self-update authenticity (#3).** The updater now refuses any release without a published
  `SHA256SUMS.txt` (previously a missing checksum silently *skipped* verification and ran the
  unverified exe), and an installed build only ever updates from Benchly's baked-in official
  source — the user-writable `update_repo` override is honoured only when running from source.
  Together these close a local privilege-escalation path (write settings.json → next update runs
  attacker code, elevated). *Authenticode signing of the released exe remains an open follow-up,
...
- **Credential-removal escaping (#5).** Removing a saved credential now passes the target as a
  PowerShell *literal* (single-quoted), so an entry whose name contains a `$(...)` subexpression
  can no longer execute on removal.
...
### Added
- **Software update check (no winget).** A new Toolbox card reads what's installed locally and
  cross-references a **curated list of common apps** (browsers, IT/dev tools, media, utilities)
  against their **official** sources — the vendor's own version endpoint, or that project's GitHub
  Releases — for a newer version. When something's behind, it shows the installed-vs-latest gap and
  a button to open the **official download page** in your browser.
  - **Deliberately winget-free.** Only plain HTTPS "what's the latest version?" lookups go out, each
    straight to the one vendor — so there's no package-manager behaviour for tight endpoint
    protection to terminate, and **no list of your installed software is sent anywhere**. It's
    read-only: Benchly never downloads or installs anything itself.
  - Coverage is the curated catalogue (matched against what you actually have installed) and grows
    over time; apps it doesn't recognise are left out rather than guessed at. Results are cached for
    six hours so re-checks don't re-hit the network.
  - Reachable from the command palette: "Check for app updates (no winget)".

## [2.13.0] — 2026-06-25

### Added
- **Hover tips throughout the app.** Almost every interactive piece of Benchly now shows a
  small, plain-English tooltip on hover (or keyboard focus) that explains what it does:
  ...
  - **Tweaks** — every toggle says what flipping it does and whether it needs a reboot or restarts
    Explorer, so nothing is a mystery before you click.
  - The ~40 existing native hints (repair-tool descriptions, copy buttons, printer actions, …) were
    folded into the same styled component, so they all look and behave consistently.
- One shared tooltip component, themed with the design-system tokens (`--overlay`, `--border-2`,
  `--text-1`, `--r1`, `--shadow-overlay`), so it fits graphite, Frosted Glass and Chevron alike.
  It opens after a short delay, flips above/below to stay on screen, hides on scroll/click, honours
  `prefers-reduced-motion`, and is exposed as `role="tooltip"` to assistive tech.

## [2.12.2] — 2026-06-25

### Fixed
- **Frosted Glass theme contrast.** Its panels were white-translucent, which *lightened* the
  gradient behind them — so on bright backgrounds (and near the corner glow) the text and the
  lighter accents, especially the yellow "warn" pills, washed out and became hard to read.
  Panels are now a translucent **dark "smoked glass"**: the customisable gradient reads as an
  ambient glow around and through them, while text and the vivid Apple accent colours sit on a
  dark backing and stay legible on any background. The corner glow was also toned down.

## [2.12.1] — 2026-06-25

### Changed
- **Chevron theme** now renders all text in lower case (visual only — copied values keep their
  real case), shows a true double-chevron glyph in the logo tile, and adds a tagline under the
  wordmark: "it looks like you're trying to troubleshoot a pc, need some help?". Other themes are
  unaffected.

## [2.12.0] — 2026-06-25

### Added
- **Chevron appearance** — a new theme in the Appearance menu: warm amber-orange on a warm-dark
  steel palette, squared corners, an uppercase wordmark, and a chevron hazard-band across the
  title bar.
- **Workplace → Applied policies** — a read-only view of every Group Policy / MDM setting
  actually applied to this machine (walked from the policy registry hives), grouped by area
  (Windows Update, Defender, Edge, Explorer…) with each setting's registry key, type and value.
  The "show me everything that's been imposed on this PC" view.
- **Workplace → Corporate IT** — the management and security agents present and their state
  (ConfigMgr/SCCM, Intune Management Extension, MDM enrolment, Defender for Endpoint, third-party
  EDR, VPN/ZTNA, backup), plus Windows Update management (WSUS server or Update-for-Business
  deferrals), system/user proxy, and managed-network identity. All read-only.

## [2.11.1] — 2026-06-25

### Fixed
- **Self-update for the machine-wide (Program Files) install** — the case that "closed but never
  installed". Updating that copy must write into Program Files, which needs elevation; the old
  flow fired a hidden `cmd.exe` UAC prompt *after* the window closed, which was easy to miss or
  decline. Now the freshly-downloaded build performs the swap itself, the elevation prompt is
  branded **Benchly** and **expected** (the UI tells you to click **Yes**), and the helper logs
  to `%TEMP%\benchly-update.log` for diagnosis. Portable builds (no elevation needed) are
  unchanged and were already reliable.
- **Why a stuck copy needs one manual update:** the updater that runs is the one baked into your
  *installed* version. A copy already on an older build (e.g. v2.7.0) can't retroactively get
  this fix — install this version once by hand, and self-update is reliable from then on.

## [2.11.0] — 2026-06-25

A hardening pass on the UI: elevation-gated features are now hidden until the app is elevated,
rather than shown disabled or labelled "needs admin".

### Changed
- **Anything that requires elevation to run is hidden from the standard (non-elevated) view** and
  only appears once you Run as admin: the repair Toolbox, admin-only tweaks, the hardening fixes
  and ASR controls, Defender exclusion removal, restore-point creation, the Workplace managed
  baseline (Apply/Clear), take-ownership, memory diagnostic, print-queue purge, Set-Private,
  machine-PATH clean, audio-service restart, VHDX compaction, BitLocker recovery-key reveal, and
  admin runbook steps. Command-palette entries for those tools are filtered out too.
- **Privileged read-only data that can't populate without elevation is also hidden** (rather than
  a "needs admin" placeholder): SMART attributes, TPM / Secure Boot / BitLocker status, advanced
  storage health, the Defender exclusion list, recent-execution (Prefetch), and the full boot
  breakdown.
- The **"Standard" badge** now explains elevated tools are hidden, and **Run as admin** reveals
  them all. The Health Audit and hardening scorecard stay visible and degrade honestly — they're
  informational audits, not actionable tools.

## [2.10.1] — 2026-06-25

### Changed
- **Treemap folders are now tinted by their dominant content type** — a folder mostly full of
  video is purple, mostly images teal, mostly code green, and so on (same palette as the File
  types view). Computed during the existing size walk, so no extra cost. Makes "what is this
  drive full of?" readable at a glance, not just "where are the big folders."

## [2.10.0] — 2026-06-25

The space analyzer goes visual.

### Added
- **Treemap view** in the Storage space analyzer — a WinDirStat-style map where every folder and
  file is a rectangle sized by the disk it uses and colour-coded by type, so the space hogs are
  obvious at a glance. Click a folder tile to drill in; hover for the exact size.
- **File types view** — aggregates every file under the current folder by extension (one walk),
  so you can see whether it's video, images, archives or logs eating the space.
- Files are colour-coded by category (images, video, audio, documents, archives, code, binaries)
  using the existing chart palette. A **Map / List / File types** toggle switches between them;
  the original List view (with reveal-in-Explorer and drill-down) remains.

## [2.9.0] — 2026-06-25

"Bundle J" — more one-stop tools, deeper diagnostics, and a UI/accessibility sweep.

### Added — tools
- **File hash** (Toolbox) — MD5 / SHA-1 / SHA-256 of any file, computed locally; pairs with the
  VirusTotal check.
- **Hosts file viewer** (Toolbox) — every active mapping in the Windows hosts file, with
  non-default entries flagged (a classic redirect/hijack check). Read-only.
- **SMART attributes** (Storage) — the raw drive self-monitoring counters (reallocated, pending
  and uncorrectable sectors flagged), read on demand. SATA/ATA mostly; NVMe rarely exposes them.
- **Crash-dump suspects** (Event Log → Crashes) — parses the minidumps to list the third-party
  drivers loaded at crash time, the usual BSOD culprits. (The Stop code itself still comes from
  the authoritative WER event.)

### Added — tweaks (now 53)
- New **Network & power** group: disable IPv6, show Hibernate in the power menu.
- **Take ownership** right-click verb (the "access denied, can't delete" fix), **This PC** on
  the desktop, and **drive letters before names** in Explorer.

### Changed — UI & accessibility (from a full design + a11y review)
- **Sidebar number shortcuts** now read as a clean 1–0 run — Security was sitting unnumbered
  inside the numbered group and interrupting it; it moved to the secondary group. The keys were
  always correct; this aligns the badges with them.
- Tweak toggles are now real **switches** to assistive tech (role / state / label), and every
  text input gets an accessible name — closing the WCAG 2.1 AA findings from the review.

## [2.8.1] — 2026-06-25

Tweaks expansion — Cleanup → Tweaks grew from 25 to **48** reversible, documented toggles.
Every one shows its exact registry location and restores the Windows default when turned off.

### Added — tweaks
- **New Gaming group**: disable mouse acceleration, Game DVR background recording, and
  multimedia network throttling.
- **Performance**: disable transparency effects, window animations, and **Fast Startup**
  (hybrid shutdown) — the "did you really do a full shutdown?" fix for dual-boot and driver oddities.
- **Privacy**: disable Activity History/Timeline, Cortana, inking & typing personalization,
  most-used-app tracking, and language-list sharing with websites.
- **Interface**: hide the Task View button, compact File Explorer view, disable Aero Shake,
  snappier menus (no show delay), expand-nav-pane-to-current-folder, show full path in the
  title bar, and add **"End task"** to the taskbar right-click (Win11 23H2+).
- **Ads & noise**: hide File Explorer promo (OneDrive) ads, "Finish setting up" nags, the
  post-update welcome page, suggested actions, and Start menu recommendations.

The new ad/noise toggles also join the one-click **"calm this computer down"** quiet mode.

## [2.8.0] — 2026-06-25

"Bundle I" — the repair bench: deeper fixes, error decoding, and an accessibility pass.

### Added
- **Error-code decoder** (Toolbox + Ctrl+K) — paste any Windows status code (`0x8007…`, an
  NTSTATUS, a Stop code, or a decimal) for plain English: the meaning (resolved via the OS
  itself, so it never goes stale), the HRESULT severity/facility breakdown, and
  Windows-Update-specific advice for `0x8024…` codes.
- **What's locking this file?** (Toolbox) — enter a file/folder path and see which processes
  hold an open handle to it — the "can't delete, it's in use" answer.
- **Full network reset** (Toolbox) — Winsock + TCP/IP stack reset, flush DNS, and release/renew
  in one guided sequence, plus a standalone TCP/IP stack reset (`netsh int ip reset`).
- **User profile health** (Toolbox) — read-only detection of corrupted (`.bak`) and temporary
  user profiles from the ProfileList registry. Migrating to a fresh account stays a manual call.
- **Re-register Store & built-in apps** (Toolbox repair tools) — the standard fix for a broken
  Start menu, Store, or built-in apps; per-user, reversible, no uninstalls.

### Changed
- **Blue screens now name the Stop code** (Event Log → Crashes) — e.g. `0x000000D1
  DRIVER_IRQL_NOT_LESS_OR_EQUAL` — and Kernel-Power 41 events that carried a bugcheck are
  surfaced as the crashes they actually were (the decimal→hex conversion is handled).
- **Wider autostart/persistence coverage** (Security → Autoruns) — adds RunServices(Once),
  policy `Explorer\Run`, RunOnceEx, LSA security/authentication providers, and print monitors.

### Accessibility
- Dim tertiary text lifted to meet **WCAG AA** contrast (4.5:1) on the card surfaces.
- The OS **"reduce motion"** setting is now honoured — decorative transitions/animations drop
  while live telemetry keeps updating.
- Icon-only buttons gained accessible names (`aria-label`).

### Deferred to a later bundle
- Named SMART-attribute decoding and minidump faulting-driver analysis — both need careful
  low-level parsing (vendor attribute bytes; dump-format walking) and are scoped for a follow-up.
...
### Security & stability
- **Domain TLS inspector** now pins a minimum of **TLS 1.2** explicitly when it connects to
  read a site's certificate. It already negotiated 1.2+ in practice; this makes the floor
  unambiguous and clears a static-analysis (CodeQL) finding.
- **Page navigation** dispatches through a `Map` of known page loaders, so a page name can
  never resolve to anything but one of the real loaders — clears a second CodeQL finding.

### Developer / tooling
- Added a pinned **`requirements.txt`** (runtime + build + audit deps) so the environment
  rebuilds identically on any machine.
- Documented the **dependency & CVE audit** routine in `docs/dependency-audit.md`, with a
  GitHub issue template and a recurring audit that files issues on real findings.

## [2.7.0] — 2026-06-14

The "power, storage & runtime forensics" release — Bundle H, the last of the
v2.4–v2.7 roadmap.

### Added
- **Battery & power efficiency** (System) — battery wear and cycle count (from
  `powercfg`'s battery report), plus a short on-demand `powercfg /energy` trace that
  names what's hurting power efficiency (selective-suspend off, devices blocking
  sleep, thirsty drivers). The trace needs admin.
- **Environment & PATH audit** (System) — reads the Machine and User PATH from the
  registry and checks every entry for a missing folder, duplicate, or quoting issue —
  the baffling "command not found / wrong version runs" fixer. One-click clean of the
  broken and duplicate entries, with the prior value backed up for undo.
- **Installed runtimes** (System) — the .NET Framework, .NET (Core/5+), Visual C++
  redistributables and DirectX a program quietly needs. The "app won't start, missing
  runtime" diagnosis in one place.
- **Advanced storage health** (Storage) — beyond SMART: whether **TRIM** is running on
  the SSDs, **Storage Spaces** pool/virtual-disk health, the drives' **reliability
  counters** (wear, temperature, read/write errors — wear/temp need admin), the
  **shadow-copy** (System Restore) space in use, and the filesystem **dirty bit**.
- **Audio device doctor** (Devices) — the "no sound / wrong output / can't pick a
  device" triage: playback and recording endpoints with their state, the audio
  services, and a one-click restart.
